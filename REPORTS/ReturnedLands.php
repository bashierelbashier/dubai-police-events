...
class MYPDF extends TCPDF {
	...
	public function Header() {
        ...
        $da = date("Y/m/d");
        $this->Cell('', 5,' الأراضي التي تم إستردادها ولم تخصص حتى تاريخ  : '.$da, 0, true, 'C', 0, '', 0, true, 'M', 'M');
        $this->ln();
        $this->ln();
        $this->ln();
        $this->SetFont('aealarabiya', 'B', 13);

        $this->Cell(180, 5,'   #             المحلية                       المربوع                         رقم القطعة                   تاريخ الإسترداد          ', 0, true, 'L', 0, '', 0, true, 'M', 'M');
        // Title
...
$query = "SELECT MAX(ID) AS ID,LAND_NO,DISTRICT_NO FROM T_TRANSACTION
GROUP BY LAND_NO,DISTRICT_NO";
...
while ($row=mysqli_fetch_array($res))
{
    ...
    $lres = mysqli_query($connect,$lquery);
    $lrow = mysqli_fetch_array($lres);

    if ($lrow['TRANSACTION_NO']==11)
        $output .='
        <tr align="center">
        <td width="25%">'.$lrow['TRANS_DATE'].'</td>
        <td width="25%">'.$lrow['LAND_NO'].'</td>
        <td width="25%">'.$lrow['DISTRICT_NAME'].'</td>
        <td width="20%">'.$lrow['LOCALE_NAME'].'</td>
        <td width="5%">'.$count++.'</td>
        </tr>';
