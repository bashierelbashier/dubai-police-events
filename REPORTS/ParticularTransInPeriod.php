...
class MYPDF extends TCPDF {
	...
	public function Header() {
        ...
        $da = date("Y/m/d");
        $this->Cell('', 5,' معاملات '.$row['TRANSACTION_NO'].' تمت في الفترة من  : '.$row['from_date'].'  إلى  '.$row['to_date'], 0, true, 'C', 0, '', 0, true, 'M', 'M');
        $this->ln();
        $this->ln();
        $this->ln();
        $this->SetFont('aealarabiya', 'B', 13);

        $this->Cell(5, 5,'            #                المحلية                              المربوع                         رقم القطعة              تاريخ المعاملة       ', 0, true, 'L', 0, '', 0, true, 'M', 'M');
        // Title  
...
$query = "SELECT * FROM T_TRANSACTION JOIN T_LANDS USING (LAND_NO,DISTRICT_NO)
    JOIN T_DISTRICTS USING (DISTRICT_NO,LOCALE_NO) JOIN T_LOCALES USING (LOCALE_NO) WHERE TRANS_DATE >= '".$_GET['from_date']."'
    AND TRANS_DATE <= '".$_GET['to_date']."' AND TRANSACTION_NO = ".$_GET['TRANSACTION_NO'];
...
while ($row=mysqli_fetch_array($result))
{
    ...
    $output .= '<tr align="center">
     <td width="20%">'.$row['TRANS_DATE'].'</td>
	 <td width="20%">'.$row['LAND_NO'].'</td>
	 <td width="30%">'.$row['DISTRICT_NAME'].'</td>
	 <td width="25%">'.$row['LOCALE_NAME'].'</td>
	 <td width="5%">'.$count.'</td>
	</tr>';
